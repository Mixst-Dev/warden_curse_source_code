...
public class CraftLoadedSpearProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double MAGNETITE_amount = 0;
		{
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.GYOUBUS_HORN.get()) {
		...
		}
		{
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					ItemStack itemstackiterator = _iitemhandlerref.get().getStackInSlot(_idx).copy();
					if (itemstackiterator.getItem() == WardenCurseModItems.MAGNETITE_SCRAP.get()) {
						MAGNETITE_amount = MAGNETITE_amount + itemstackiterator.getCount();
					}
				}
			}
		}
		if (IRON_SCRAP_amouint >= 8 && MAGNETITE_amount >= 6 && final_amount_axe >= 1) {
			...
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(WardenCurseModItems.MAGNETITE_SCRAP.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 6, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(WardenCurseModItems.GYOUBUS_HORN.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(WardenCurseModItems.LOADEDSPEARICON.get()).copy();
