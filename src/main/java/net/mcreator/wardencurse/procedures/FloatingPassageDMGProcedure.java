...
public class FloatingPassageDMGProcedure {
					...
					if (!(entity == entityiterator) && !(entityiterator instanceof TamableAnimal _tamIsTamedBy && entity instanceof LivingEntity _livEnt ? _tamIsTamedBy.isOwnedBy(_livEnt) : false) && !(entityiterator instanceof ItemEntity)
						...
						if (!(entityiterator instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
							entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK), entity),
									(float) (WardenCurseModVariables.MapVariables.get(world).config_combat_power * 0.6 * vent_power * sugar_power));
							entityiterator.setDeltaMovement(new Vec3((entity.getDeltaMovement().x() + entity.getLookAngle().x * 0.3), 0, (entity.getDeltaMovement().z() + entity.getLookAngle().z * 0.3)));
							if (entity instanceof LivingEntity _livEnt25 && _livEnt25.hasEffect(WardenCurseModMobEffects.FIRESTATE.get())) {
						...
						if (!(entityiterator instanceof LivingEntity _livEnt30 && _livEnt30
