...
public class CraftLongsparkProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double MAGNETITE_amount = 0;
		{
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.SHINOBIFIRECRACKER.get()) {
		...
		if (IRON_SCRAP_amouint >= 16 && MAGNETITE_amount >= 16 && final_amount_axe >= 1) {
			...
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(WardenCurseModItems.LONGSPARK.get()).copy();
