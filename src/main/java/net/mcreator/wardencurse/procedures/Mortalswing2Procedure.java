...
public class Mortalswing2Procedure {
		...
		}
		{
			...
			if (!_ent.level().isClientSide() && _ent.getServer() != null) {
				_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
						_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), ("summon warden_curse:mortaldraw_2 " + x2 + " ~1.2 " + z2 + " {Rotation:[" + myyaw + "f," + 0 + "f]}"));
			}
		}
		for (int index0 = 0; index0 < 10; index0++) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = WardenCurseModEntities.MORTALVISUAL.get().spawn(_level,
						BlockPos.containing(xcord + ((Math.cos(yaw) * Math.cos(pitch)) / magnitude) * 6, (y + 8) - height, zcord + ((Math.sin(yaw) * Math.cos(pitch)) / magnitude) * 6), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
					entityToSpawn.setDeltaMovement(0, 0, 0);
				}
			}
			{
				final Vec3 _center = new Vec3((xcord + ((Math.cos(yaw) * Math.cos(pitch)) / magnitude) * 6), ((y + 8) - height), (zcord + ((Math.sin(yaw) * Math.cos(pitch)) / magnitude) * 6));
