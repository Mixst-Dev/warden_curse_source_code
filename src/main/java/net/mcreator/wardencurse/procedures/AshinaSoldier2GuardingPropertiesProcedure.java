...
public class AshinaSoldier2GuardingPropertiesProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, DamageSource damagesource, Entity entity, double amount) {
		...
		if (entity instanceof AshinaSoldier2Entity) {
			if (amount > 4 && !damagesource.is(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("warden_curse:firecracker")))) {
				if (Math.random() < 0.55 && (damagesource.is(DamageTypes.ARROW) || damagesource.is(DamageTypes.EXPLOSION) || damagesource.is(DamageTypes.FIREBALL) || damagesource.is(DamageTypes.FIREWORKS)
						|| damagesource.is(DamageTypes.LIGHTNING_BOLT) || damagesource.is(DamageTypes.MOB_ATTACK) || damagesource.is(DamageTypes.MOB_ATTACK_NO_AGGRO) || damagesource.is(DamageTypes.MOB_PROJECTILE)
