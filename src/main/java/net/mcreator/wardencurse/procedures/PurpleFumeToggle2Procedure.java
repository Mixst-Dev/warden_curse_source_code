...
public class PurpleFumeToggle2Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		yaw = (entity.getPersistentData().getDouble("fire_cracker_yaw") + 90) * (Math.PI / 180);
		{
			final Vec3 _center = new Vec3((xcord + ((Math.cos(yaw) * Math.cos(pitch)) / magnitude) * 8), (ycord + (Math.sin(pitch) / magnitude) * (-1) + 1.5), (zcord + ((Math.sin(yaw) * Math.cos(pitch)) / magnitude) * 8));
				...
				if (!(entity == entityiterator) && !(entityiterator instanceof TamableAnimal _tamIsTamedBy && entity instanceof LivingEntity _livEnt ? _tamIsTamedBy.isOwnedBy(_livEnt) : false) && !(entityiterator instanceof ItemEntity)
					...
					if (!(entityiterator instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
						if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(WardenCurseModMobEffects.STUNNED.get(), 33, 1, false, false));
						entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("warden_curse:firecracker")))), (float) 0.1);
		...
		if (world instanceof ServerLevel _level) {
			Entity entityToSpawn = WardenCurseModEntities.PNFC.get().spawn(_level,
					BlockPos.containing(xcord + ((Math.cos(yaw) * Math.cos(pitch)) / magnitude) * 8, ycord + (Math.sin(pitch) / magnitude) * (-1) + 1.5, zcord + ((Math.sin(yaw) * Math.cos(pitch)) / magnitude) * 8), MobSpawnType.MOB_SUMMONED);
