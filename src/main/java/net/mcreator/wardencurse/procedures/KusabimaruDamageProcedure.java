...
public class KusabimaruDamageProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity.onGround()) {
			entity.setDeltaMovement(new Vec3((entity.getDeltaMovement().x() + entity.getLookAngle().x * 0.32), 0, (entity.getDeltaMovement().z() + entity.getLookAngle().z * 0.32)));
		}
		{
			...
			for (Entity entityiterator : _entfound) {
				if (!(entityiterator == entity) && !(entityiterator instanceof ItemEntity) && !(entity instanceof LivingEntity _livEnt18 && _livEnt18.hasEffect(WardenCurseModMobEffects.STUNNED.get()))
					...
					if (!(entity instanceof LivingEntity _livEnt20 && _livEnt20.hasEffect(WardenCurseModMobEffects.FIRESTATE.get()))
							&& !(entityiterator instanceof LivingEntity _livEnt21 && _livEnt21.hasEffect(WardenCurseModMobEffects.OIL_EFFECT.get()))) {
						entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK), entity),
								(float) ((WardenCurseModVariables.MapVariables.get(world).config_combat_power - 0.5) * vent_power * sugar_power));
					} else {
						entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK), entity),
								(float) ((WardenCurseModVariables.MapVariables.get(world).config_combat_power - 0.5) * vent_power * sugar_power * 1.15));
					...
					if (entityiterator instanceof LivingEntity _livEnt34 && _livEnt34.hasEffect(WardenCurseModMobEffects.PARRY.get())) {
						if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(WardenCurseModMobEffects.ATTACKCOOLDOWN.get(), 20, 1, false, false));
					}
					if (entityiterator.getPersistentData().getBoolean("sekiro_entity_guard") == false || !(entityiterator instanceof LivingEntity _livEnt37 && _livEnt37.hasEffect(WardenCurseModMobEffects.PARRY.get()))
