...
public class Cloud2Procedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double myyaw = 0;
		double mypitch = 0;
		if (entity.getPersistentData().getBoolean("sekiro_cloud_2") == true) {
			entity.getPersistentData().putDouble("cloud_2", (entity.getPersistentData().getDouble("cloud_2") + 1));
			x2 = entity.getX();
			z2 = entity.getZ();
			myyaw = entity.getYRot();
			mypitch = entity.getXRot();
			if (entity.getPersistentData().getDouble("cloud_2") == 1) {
				if (world.isClientSide()) {
					if (entity instanceof AbstractClientPlayer player) {
						...
						if (animation != null) {
							animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation("warden_curse", "spiral2"))));
						}
					}
				}
				if (!world.isClientSide()) {
						...
						synchronized (connections) {
							...
							while (iterator.hasNext()) {
								...
								if (!connection.isConnecting() && connection.isConnected())
									WardenCurseMod.PACKET_HANDLER.sendTo(new StateanimationProcedure.WardenCurseModAnimationMessage(Component.literal("spiral2"), entity.getId(), true), connection, NetworkDirection.PLAY_TO_CLIENT);
							}
						}
					}
				}
				...
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(WardenCurseModMobEffects.TECHNIQUECOOLDOWN.get(), 20, 1, false, false));
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
			...
			if (entity.getPersistentData().getDouble("cloud_2") == 13) {
			...
			if (entity.getPersistentData().getDouble("cloud_2") == 17) {
				if ((entity.getCapability(WardenCurseModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new WardenCurseModVariables.PlayerVariables())).Cancel_Art_Toggle == true) {
					{
						Entity _ent = entity;
						if (!_ent.level().isClientSide() && _ent.getServer() != null) {
			...
			if (entity.getPersistentData().getDouble("cloud_2") == 20) {
				if ((entity.getCapability(WardenCurseModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new WardenCurseModVariables.PlayerVariables())).Cancel_Art_Toggle == true) {
			...
			if (entity.getPersistentData().getDouble("cloud_2") == 30) {
				entity.getPersistentData().putDouble("spiral_number", 0);
				entity.getPersistentData().putDouble("cloud_2", 0);
				entity.getPersistentData().putBoolean("sekiro_cloud_2", false);
			}
		}
	}
}
