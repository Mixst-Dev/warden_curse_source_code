...
public class CraftLeapingFireProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double FULIMATED_amount = 0;
		double BLACK_amount = 0;
		double YELLOW_amount = 0;
		double ADAMANTITE_amount = 0;
		{
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.LOADEDSPEARICON.get()) {
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.FULIMATED_MERCURY.get()) {
						FULIMATED_amount = FULIMATED_amount + itemstackiterator.getCount();
					}
				}
			}
		}
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.BLACK_GUNPOWDER.get()) {
						BLACK_amount = BLACK_amount + itemstackiterator.getCount();
					}
				}
			}
		}
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.YELLOW_GUNPOWDER.get()) {
						YELLOW_amount = YELLOW_amount + itemstackiterator.getCount();
					}
				}
			}
		}
		if (FULIMATED_amount >= 2 && BLACK_amount >= 16 && YELLOW_amount >= 8 && ADAMANTITE_amount >= 4 && final_amount_axe >= 1) {
			...
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(WardenCurseModItems.LEAPINGFLAME.get()).copy();
