...
public class ACVHardPunch1Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		zcord = entity.getZ();
		{
			final Vec3 _center = new Vec3((xcord + ((Math.cos(yaw) * Math.cos(pitch)) / magnitude) * 2 * 2), (ycord + (Math.sin(pitch) / magnitude) * (-1) + 1.5), (zcord + ((Math.sin(yaw) * Math.cos(pitch)) / magnitude) * 2 * 2));
			...
			for (Entity entityiterator : _entfound) {
				if (!(entity == entityiterator) && !(entityiterator instanceof ItemEntity) && entityiterator == (entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null)) {
					entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("warden_curse:powerfullhit")))), 33);
					entityiterator.setDeltaMovement(new Vec3((entity.getDeltaMovement().x() + entity.getLookAngle().x * 3.4), 0.2, (entity.getDeltaMovement().z() + entity.getLookAngle().z * 3.4)));
					...
					if (entityiterator instanceof LivingEntity _livEnt18 && _livEnt18.hasEffect(WardenCurseModMobEffects.PARRY.get())) {
						entity.getPersistentData().putDouble("entity_posture", (entity.getPersistentData().getDouble("entity_posture") + 90));
					}
