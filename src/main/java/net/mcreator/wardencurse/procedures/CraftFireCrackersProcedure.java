...
public class CraftFireCrackersProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double MAGNETITE_amount = 0;
		{
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.ROBERT_FIRECRACKERS.get()) {
		...
		if (IRON_SCRAP_amouint >= 4 && MAGNETITE_amount >= 14 && final_amount_axe >= 1) {
			...
			if (entity instanceof Player _player) {
				...
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 14, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(WardenCurseModItems.ROBERT_FIRECRACKERS.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(WardenCurseModItems.SHINOBIFIRECRACKER.get()).copy();
