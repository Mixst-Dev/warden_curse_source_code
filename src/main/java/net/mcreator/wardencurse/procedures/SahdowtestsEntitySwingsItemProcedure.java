...
public class SahdowtestsEntitySwingsItemProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		for (int index0 = 0; index0 < 20; index0++) {
			yaw = (entity.getYRot() + 90 + rotation) * (Math.PI / 180);
			{
				final Vec3 _center = new Vec3((xcord + ((Math.cos(yaw) * Math.cos(pitch)) / magnitude) * 3.5), (ycord + (Math.sin(pitch) / magnitude) * (-1)), (zcord + ((Math.sin(yaw) * Math.cos(pitch)) / magnitude) * 3.5));
					...
					if (!(entityiterator instanceof ShadowswipeEntity) && !(entity == entityiterator)) {
						if (!(entity == entityiterator) && !(entityiterator instanceof TamableAnimal _tamIsTamedBy && entity instanceof LivingEntity _livEnt ? _tamIsTamedBy.isOwnedBy(_livEnt) : false) && !(entityiterator instanceof ItemEntity)
							...
							if (!(entityiterator instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
								...
								if (entity instanceof LivingEntity _livEnt22 && _livEnt22.hasEffect(WardenCurseModMobEffects.FIRESTATE.get())) {
							...
							if (entityiterator instanceof LivingEntity _livEnt24 && _livEnt24.hasEffect(WardenCurseModMobEffects.PARRY.get())
									&& !(entityiterator instanceof LivingEntity _livEnt25 && _livEnt25.hasEffect(WardenCurseModMobEffects.STUNNED.get()))) {
								if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
									_entity.addEffect(new MobEffectInstance(WardenCurseModMobEffects.PARRYVISUAL.get(), 10, 1, false, false));
							}
							if (!(entityiterator instanceof LivingEntity _livEnt27 && _livEnt27.hasEffect(
		...
		if (world instanceof ServerLevel _level) {
			Entity entityToSpawn = WardenCurseModEntities.SHADOWSPIN.get().spawn(_level, BlockPos.containing(xcord, ycord, zcord), MobSpawnType.MOB_SUMMONED);
