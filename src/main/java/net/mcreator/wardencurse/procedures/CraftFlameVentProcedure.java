...
public class CraftFlameVentProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double MAGNETITE_amount = 0;
		{
			...
			if (_iitemhandlerref.get() != null) {
				for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
					...
					if (itemstackiterator.getItem() == WardenCurseModItems.FLAME_BARREL.get()) {
		...
		if (IRON_SCRAP_amouint >= 6 && MAGNETITE_amount >= 8 && final_amount_axe >= 1) {
			...
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(WardenCurseModItems.FLAME_BARREL.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(WardenCurseModItems.FIREVENT_1.get()).copy();
